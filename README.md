...
- **Experimental sensitive-source attestation** — `SAP_SENSITIVE_DATA_SOURCES=KNA1,LFA1` pauses a direct read of a listed table with `DATA_SOURCE_SENSITIVE` until the caller repeats it with a `justification`, which ARC-1 records in the audit log together with the matched sources and the calling user before running the request. Exact direct names only, zero SAP calls, works on every SAP release; accountability rather than a boundary — sources nobody may read belong in the blocklist, which wins on overlap.
...
| Exact sensitive list with audited justification (experimental) | `SAP_SENSITIVE_DATA_SOURCES=KNA1,LFA1` | empty (off) | Pauses direct reads of listed sources until a `justification` is supplied and recorded with the user; not a denial, zero SAP calls. |
