...
| Audit logging / new audit event type | `src/server/audit.ts` (typed `*Event` union; emit via `logger.emitAudit`), `src/server/sinks/` — the BTP sink is mTLS-only: the `auditlog` premium binding must be x509 (`certurl`/`certificate`/`key`), a default `binding-secret` binding is rejected at startup (`BTPAuditLogBindingError`), never reported as enabled |
