...
| Exact sensitive list with audited justification (experimental) | `SAP_SENSITIVE_DATA_SOURCES=KNA1,LFA1` | empty (off) | Pauses direct reads of listed sources until the caller supplies a `justification`, then records it with the user and runs. Accountability, not a boundary; direct sources only, zero SAP calls, every release. |
...
| `SAP_SENSITIVE_DATA_SOURCES`       | Exact sensitive sources whose reads should be justified and audited rather than denied | Experimental, default-off attestation control: direct-source exact match, zero SAP calls, works on SAP_BASIS 7.50. Records the caller's `justification` and identity in the audit log; it cannot prove a human agreed. Sources nobody may read belong in `SAP_BLOCKED_DATA_SOURCES`. |
